...
namespace assignment3
{
	...
	private:
		...
		void addNew(Node<T>* current, T& student);
		void fail(Node<T>* current, float grade);
		void sum(Node<T>* current);
		void remove(Node<T>* &p);

	public:
		//Constructors
		BinaryTree();
		BinaryTree(T& student);
		...
		T treeRoot() const;
		int compare(T a, T b);
		int compareGrade(float a, float b);
		void printInOrder();
		void fail(float grade);
		std::size_t ffs();
		void sum();
		float average();
	...
	void BinaryTree<T>::inOrder(Node<T>* node)
	{
		if (node != NULL)
		{
			...
			std::cout << node->getStudent().getGrade() << " ) ";
	...
	void BinaryTree<T>::findStudent(std::string name)
	{
		...
		else
		{
			...
			else if (found)
			{
				...
				std::cout << current->getStudent().getGrade() << " ) " << std::endl;
			}
		}
	}
}
